    ...
    if(emptyInputs.length!=0) {
        ...
        fetch("https://json-api.uz/api/project/game-over/auth/login",{
            ...
            if(res=="User not found (check username and password)"){
                ...
                let userResponse = JSON.parse(res);
                localStorage.setItem("access_token",userResponse.access_token);
                localStorage.setItem("user_name",userResponse.username);
...
elRegisterForm.addEventListener("submit",(evt)=>{
    ...
    if(emptyInputs.length!=0) {
        ...
        fetch("https://json-api.uz/api/project/game-over/auth/register",{
            method:"POST",
            headers: {
                "Content-Type":"application/json"
            ...
            if(res=="User not found (check username and password)"){
                ...
                let userResponse = JSON.parse(res);
                localStorage.setItem("test",JSON.stringify(userResponse));
                localStorage.setItem("access_token",userResponse.access_token);
                localStorage.setItem("user_name",userResponse.username);
